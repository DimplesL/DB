...
class ImageDataset(data.Dataset, Configurable):
    ...
    def get_all_samples(self):
        for i in range(len(self.data_dir)):
            ...
            if self.is_training:
                image_path = [self.data_dir[i] + '/train/imgs/' + timg.strip() for timg in image_list]
                gt_path = [self.data_dir[i] + '/train/gt/' + 'gt_' + timg.strip().replace('jpg', 'txt') for timg in image_list]
            else:
                image_path = [self.data_dir[i] + '/test/imgs/' + timg.strip() for timg in image_list]
                print(self.data_dir[i])
                if 'TD500' in self.data_list[i] or 'total_text' in self.data_list[i]:
                    gt_path = [self.data_dir[i] + '/test/gt/' + timg.strip() + '.txt' for timg in image_list]
                else:
                    gt_path = [self.data_dir[i] + '/test/gt/' + 'gt_' + timg.strip().replace('jpg', 'txt') for timg in
                               image_list]
